...
#define MG_LOAD_BE24(p) ((uint32_t)((MG_U8P(p)[0] << 16U) | (MG_U8P(p)[1] << 8U) | MG_U8P(p)[2]))
...
#define GCM_AUTH_FAILURE 0x55555555  // authentication failure
  ...
  //
  //
  int mg_aes_gcm_encrypt(unsigned char *output, const unsigned char *input, size_t input_length,
                         const unsigned char *key, const size_t key_len, const unsigned char *iv, const size_t iv_len,
                         unsigned char *aead, size_t aead_len, unsigned char *tag, const size_t tag_len);

  int mg_aes_gcm_decrypt(unsigned char *output, const unsigned char *input, size_t input_length,
                         const unsigned char *key, const size_t key_len, const unsigned char *iv, const size_t iv_len);
...
#define MG_UECC_SUPPORT_COMPRESSED_POINT 1
  ...
  extern "C"
  {
    ...
    */
    ...
    occurred.
    */
    int mg_uecc_shared_secret(const uint8_t *public_key, const uint8_t *private_key, uint8_t *secret,
                              MG_UECC_Curve curve);
    ...
    */
    int mg_uecc_verify(const uint8_t *public_key, const uint8_t *message_hash, unsigned hash_size,
                       const uint8_t *signature, MG_UECC_Curve curve);
...
#define TLS_RSA_H
  ...
  int mg_rsa_mod_pow(const uint8_t *mod, size_t modsz, const uint8_t *exp, size_t expsz, const uint8_t *msg,
                     size_t msgsz, uint8_t *out, size_t outsz);
  ...
  struct mg_timer *mg_timer_add(struct mg_mgr *mgr, uint64_t milliseconds, unsigned flags, void (*fn)(void *),
                                void *arg);
