...
Pixelcade* Pixelcade::Open(const char* pDevice, int width, int height) {
  ...
  result = sp_blocking_read(pSerialPort, response, 29,
                            PIXELCADE_COMMAND_READ_TIMEOUT);
  if (response[0] != PIXELCADE_RESPONSE_ESTABLE_CONNECTION) {
    sp_close(pSerialPort);
    sp_free_port(pSerialPort);
    // Log("Pixelcade: expected new connection to return 0x0, but got 0x%02d",
    // response[0]);
    return nullptr;
  }

  if (response[1] != 'I' || response[2] != 'O' || response[3] != 'I' ||
      response[4] != 'O') {
    sp_close(pSerialPort);
    sp_free_port(pSerialPort);
    // Log("Pixelcade: expected magic code to equal IOIO but got %c%c%c%c",
  ...
  Log("Pixelcade found: device=%s, Hardware ID=%s, Bootloader ID=%s, "
      "Firmware=%s",
      pDevice, hardwareId, bootloaderId, firmware);
    ...
    while (m_running) {
      ...
      if (pFrame) {
        ...
        if (m_width == 128 && m_height == 32)
          FrameUtil::SplitIntoRgbPlanes(pFrame, 128 * 32, 128, 16,
                                        (uint8_t*)planes);
        else {
          uint16_t scaledFrame[128 * 32];
          FrameUtil::ResizeRgb565Bilinear(pFrame, m_width, m_height,
                                          scaledFrame, 128, 32);
          FrameUtil::SplitIntoRgbPlanes(scaledFrame, 128 * 32, 128, 16,
                                        (uint8_t*)planes);
        }

        static uint8_t command = PIXELCADE_COMMAND_RGB_LED_MATRIX_FRAME;
        sp_blocking_write(m_pSerialPort, &command, 1,
                          PIXELCADE_COMMAND_WRITE_TIMEOUT);

        enum sp_return response =
            sp_blocking_write(m_pSerialPort, planes, 128 * 32 * 3 / 2,
                              PIXELCADE_COMMAND_WRITE_TIMEOUT);
        ...
        if (response > 0) {
          ...
          if (errors++ > PIXELCADE_MAX_NO_RESPONSE) {
            Log("Error while transmitting to Pixelcade: no response for the "
                "past %d frames",
